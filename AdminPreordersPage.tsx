...
export function AdminPreordersPage() {
  ...
  const deletePreorder = async (r: PreorderWithCollection) => {
    const confirmed = window.confirm(
      `Delete the preorder from "${r.full_name}"? This will also remove its row from Google Sheets (if connected) and give the piece back to the collection's stock. This cannot be undone.`
    );
    if (!confirmed) return;
    setDeletingId(r.id);
    try {
      ...
      const res = await fetch(apiUrl, {
        ...
        body: JSON.stringify({ action: 'delete-preorder', preorderId: r.id }),
      });
      ...
      setDeletingId(null);
      window.alert('Failed to delete preorder: ' + (e instanceof Error ? e.message : String(e)));
      return;
    }
    setDeletingId(null);
