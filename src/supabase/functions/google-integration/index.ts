...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (action === "idea") {
      ...
      if (!idea) return json({ error: "Missing idea payload." }, 400);
      if (emailEnabled && settings.gmail_address) {
        try {
          await sendIdeaNotificationEmail(settings, idea);
        } catch (e) {
          console.error("[google-integration] Idea email error:", e.message);
          return json({ message: "Idea saved, but the email notification failed.", errors: [e.message] }, 207);
        }
      }
      return json({ message: "Idea processed successfully." });
    }
...
async function sendIdeaNotificationEmail(settings: Record<string, unknown>, idea: Record<string, unknown>) {
  const resendKey = Deno.env.get("RESEND_API_KEY");
  if (!resendKey) {
    console.error("[email] RESEND_API_KEY not set — skipping email");
    return;
  ...
  const name = String(idea.name ?? "Anonymous");
  const contact = idea.contact ? String(idea.contact) : "";
  const message = String(idea.message ?? "");
  const date = new Date().toLocaleString("en-US", {
    year: "numeric", month: "long", day: "numeric",
    hour: "2-digit", minute: "2-digit",
  });

  const subject = `New Collection Idea — ${name}`;
  const html = `
    <div style="font-family: Georgia, 'Times New Roman', serif; max-width: 560px; margin: 0 auto; background: #F8F5EF; padding: 32px 28px; border: 1px solid #E8E3D8;">
      <p style="font-family: Arial, sans-serif; font-size: 11px; letter-spacing: 2px; text-transform: uppercase; color: #D9C5A1; margin: 0 0 8px;">be — Collection Idea</p>
      <h1 style="font-size: 22px; font-weight: 400; color: #0F1C2E; margin: 0 0 24px;">Someone shared a collection idea</h1>
      <table style="width: 100%; border-collapse: collapse; font-family: Arial, sans-serif; font-size: 13px; color: #1C1C1C;">
        <tr><td style="padding: 8px 0; color: #6B6B6B; width: 110px;">Name</td><td style="padding: 8px 0;">${escapeHtml(name)}</td></tr>
        ${contact ? `<tr><td style="padding: 8px 0; color: #6B6B6B;">Contact</td><td style="padding: 8px 0;">${escapeHtml(contact)}</td></tr>` : ""}
        <tr><td style="padding: 8px 0; color: #6B6B6B;">Date</td><td style="padding: 8px 0;">${date}</td></tr>
      </table>
      <div style="margin-top: 20px; padding: 16px; background: #FFFFFF; border: 1px solid #E8E3D8; border-radius: 12px; font-family: Arial, sans-serif; font-size: 13px; line-height: 1.6; color: #1C1C1C; white-space: pre-wrap;">${escapeHtml(message)}</div>
      <p style="margin-top: 28px; font-family: Arial, sans-serif; font-size: 11px; color: #6B6B6B;">View and manage all ideas from your admin panel → Ideas.</p>
    </div>
  `;
