          ...
          return (
            ...
            >
              ...
              <div className="relative flex h-full flex-col p-6 md:p-7">
                {settings.explore_cover_text_visible !== false && (
                  <>
                    <span
                      className="font-serif text-[1.1rem] font-light"
                      style={{ color: settings.explore_cover_text_color || undefined }}
                    >
                      <span className={settings.explore_cover_text_color ? '' : 'text-bg/90'}>{brandPrefix}</span>
                    </span>
                    <span
                      className={`mt-0.5 font-serif font-light leading-[0.95] ${
                        settings.explore_cover_text_color ? '' : WORD_COLORS[i % WORD_COLORS.length]
                      }`}
                      style={{
                        fontSize: 'clamp(1.9rem, 6vw, 2.6rem)',
                        color: settings.explore_cover_text_color || undefined,
                      }}
                    >
                      {coverWord(c.name)}
                    </span>
